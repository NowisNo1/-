...
namespace Garbagemanage.DAL.Base
{
    public class DbConvert
    {
        ...
        public static List<T> DataReaderToList<T>(SqlDataReader dr, string cols, string real_cols="")
        {
            
            List<T> list = new List<T>();
            PropertyInfo[] properties = PropertyHelper.GetProperties<T>(cols, real_cols);
            if (!string.IsNullOrEmpty(real_cols))
            {
                foreach(PropertyInfo p in properties)
                {
                    Console.WriteLine(p.GetColName());
                }
            }
            if (dr != null)
            {
